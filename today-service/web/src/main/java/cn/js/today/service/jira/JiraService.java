...
//    private ConfigMapper configMapper;
    ...
    //用于存放所有的UserWorklog信息
//    List<UserWorklog> userWorklogList = new LinkedList<>();
    /**
     *
            ...
            for(int i = 0; i < issuesJsonArray.size(); i++){
                ...
                try {
                    ...
                    while (worklogIterator.hasNext()){
                        ...
                        DateTime worklogCreationDate = worklog.getCreationDate();
                        Integer worklogMinutesSpent = worklog.getMinutesSpent();
                        BasicUser worklogUser = worklog.getAuthor();
                        String worklogUserDisplayName = worklogUser.getDisplayName();
                        String worklogDateTimeStr =  DateUtil.formatDate(worklogDateTime.toDate());
                        log.info("-----------===============worklogDateTimeStr:" + worklogDateTimeStr);
                        log.info("-----------===============worklogDateTime:" + worklogDateTime);
                        log.info("-----------===============worklogUserDisplayName:" + worklogUserDisplayName);
                        log.info("-----------===============worklogMinutesSpent:" + worklogMinutesSpent);
                        log.info("-----------===============worklogCreationDate:" + worklogCreationDate);
...
//                jiraIssueDTO.setReleaseDate(releaseDate);
    ...
    public List<PersionWorklog> getWorklogSpent(String jiraURL, String loginUserName, String loginPassword, String userName, String beginDate, String endDate){
        String jiraURL3 = "http://jira.cmvideo.cn/rest/api/latest/search?jql=worklogAuthor%20=%20" + userName + "%20AND%20worklogDate%20%3E=%20%22" + beginDate + "%22%20AND%20worklogDate%20%3C=%20%22" + endDate + "%22";
        ...
        String httpResponseStr = httpResponse.body();
        log.info("httpResponseStr:" + httpResponseStr);
        List<PersionWorklog> persionWorklogList = new LinkedList<>();
        JSONObject jsonObject = JSONUtil.parseObj(httpResponseStr);
        Integer total = (Integer)jsonObject.get("total");
        Integer maxResults = (Integer)jsonObject.get("maxResults");
        JSONArray issueJsonArray = JSONUtil.createArray();
        UserWorklog returnUserWorklog = null;
        if(total > 0){
            JSONArray issuesJsonArray = (JSONArray)jsonObject.get("issues");
            for(int i = 0; i < issuesJsonArray.size(); i++){
                JSONObject issueJsonObject = (JSONObject)issuesJsonArray.get(i);
...
//                    List<String> worklogStartDateList = new ArrayList<>();
                    while (worklogIterator.hasNext()){
                        ...
                        Map<String,Float> worklogDailySpent = new LinkedHashMap<>();
                        Worklog worklog = (Worklog)worklogIterator.next();
                        BasicUser worklogUser = worklog.getAuthor();
                        String worklogUserDisplayName = worklogUser.getDisplayName();
                        ...
                        PersionWorklog persionWorklog  = new PersionWorklog();
                        persionWorklog.setUsername(worklogUserDisplayName);
                        persionWorklog.setWorklogDate(worklogDateTimeStr);
                        persionWorklog.setWorklogHours(worklogMinutesSpent);

                        persionWorklogList.add(persionWorklog);
//                        userWorklog.setUsername(worklogUserDisplayName);
//                        worklogDailySpent.put(worklogDateTimeStr,Float.valueOf(worklogMinutesSpent));
//                        userWorklog.setEverydaySpentTime(worklogDailySpent);
...
//                        worklogStartDateList.add(worklogDateTimeStr);
            ...
            JSONArray jsonArray=new JSONArray(persionWorklogList);
            log.info("&&&&&&^^^%%% -------:" + jsonArray);
//            returnUserWorklog = parseWorklogList(persionWorklogList);
//            log.info("&&&&&&^^^%%% -------2222:" + returnUserWorklog);
        }
        return persionWorklogList;
    }
    ...
    public List<PersionWorklog> getWorklogRows(String jiraURL, String loginUserName, String loginPassword, String userName, String beginDate, String endDate){

        List<PersionWorklog> persionWorklogList = getWorklogSpent(jiraURL, loginUserName, loginPassword, userName, beginDate, endDate);
        return persionWorklogList;
    }
...
//            }
//        }
//        userWorklog.setEverydaySpentTime(erverydaySpentTime);
